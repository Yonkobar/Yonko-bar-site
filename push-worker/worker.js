...
export default {
  async fetch(request, env) {
    // No public route/workers.dev URL. Only the Pages service binding can reach this.
...
export class PushState {
  ...
  fetch(request) {
    // Serialize read/modify/write across awaited network I/O within this object.
  ...
  async save(s) {
    // Chunk entries: a DO storage value is limited to 128 KiB. A booking history
    // or retry outbox must not eventually overflow a single JSON value.
  ...
  async handle(request) {
    ...
    if (path === '/config' && request.method === 'GET') {
      return json({publicKey: this.env.VAPID_PUBLIC_KEY, quoteAcceptanceDetectable: false, lastTick: state.lastTick || null, lastDaily: state.lastDaily});
    }
    ...
    if (path === '/test' && request.method === 'POST') {
      ...
      this.enqueue(state, tag, {title: 'Yonko Bar · Test push', body: 'Le serveur a envoyé cette notification.', url: '/dashboard', tag}, [id]);
      await this.save(state);
      await this.deliver(state);
  ...
  async tick(state, time) {
    ...
    if (isFirst) {
      // Establish a baseline: never notify historical bookings on installation.
    ...
    // The hourly window catches delayed cron runs. One durable marker per Paris date.
    ...
    await this.save(state); // Persist outbox and markers atomically before sending.
    await this.deliver(state);
  }
  async deliver(state) {
    const now = Date.now();
    ...
    await Promise.all(due.map(async ([key, job]) => {
      ...
      if (!sub || job.expires <= now) { delete state.jobs[key]; return; }
      try {
        const payload = await buildPushPayload({data: job.payload, options: {ttl: 3600, urgency: 'normal'}}, sub, {
          subject: this.env.VAPID_SUBJECT, publicKey: this.env.VAPID_PUBLIC_KEY, privateKey: this.env.VAPID_PRIVATE_KEY,
        });
        const response = await fetch(sub.endpoint, {...payload, redirect: 'error', signal: AbortSignal.timeout(8000)});
        if (response.status === 404 || response.status === 410) {
          delete state.subscriptions[job.subId]; delete state.jobs[key];
        } else if (response.ok) { delete state.jobs[key]; }
        else { throw new Error(`Push service HTTP ${response.status}`); }
      } catch {
        // Retry without exposing subscription URLs or keys to logs.
        job.attempts++;
